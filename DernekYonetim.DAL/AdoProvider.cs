    ...
    public class AdoProvider
    {
        ...
        public int ExecuteNonQuery(string commandText,Dictionary<string,object>parameters=null)
        {
            ...
            if (parameters != null)
            {
                ...
                {
                    if (command.CommandText.Contains(item.Key)) command.Parameters.AddWithValue(item.Key, item.Value);
                    else throw new ArgumentException(string.Format("Böyle bir parametre bulunamadı:{0}", item.Key));
                }
            }
        ...
        public T ExecuteScalar<T>(string commandText, Dictionary<string, object> parameters=null)    //generic metot örneği olmuş oldu bu da bize 
        {
            SqlCommand command = new SqlCommand(commandText, connection);
            if (parameters != null)
            {
